...
class VoyagerBaseController extends Controller
{
    ...
    public function deleteBreadImages($data, $rows, $single_image = null)
    {
        ...
        foreach ($rows as $row) {
            if (in_array($row->type, ['image', 'multiple_images'])) {
                $images_to_remove = ($row->type == 'multiple_images')
                    ? (json_decode($data->getOriginal($row->field)) ?? [])
                    : [$data->getOriginal($row->field)];

                foreach ($images_to_remove as $image) {
                    // Remove only $single_image if we are removing from bread edit
                    if ($image && $image != config('voyager.user.default_avatar') && (is_null($single_image) || $single_image == $image)) {
                        // --- CUSTOM DELETION LOGIC FOR AVIF/WEBP VERSIONS ---
                        try {
                            // The stored path is for the 'normal' version (e.g., .../basename.avif)
                            // We get the base path and name without extension
                            $basePath = pathinfo($image, PATHINFO_DIRNAME) . '/' . pathinfo($image, PATHINFO_FILENAME);

                            // These are the versions defined in StorageController
                            $versions = [
                                ['suffix' => '', 'format' => 'avif'],
                                ['suffix' => '-banner', 'format' => 'avif'],
                                ['suffix' => '-medium', 'format' => 'webp'],
                                ['suffix' => '-small', 'format' => 'webp'],
                                ['suffix' => '-cropped', 'format' => 'webp'],
                            ];

                            foreach ($versions as $version) {
                                $fileToDelete = $basePath . $version['suffix'] . '.' . $version['format'];
                            ...
                            \Log::error('Error deleting image versions for '.$image.': '.$e->getMessage());
                        }
                        // --- END CUSTOM DELETION LOGIC ---
                    }
                }
            }
        }
