                                        ...
                                        @foreach($dataType->browseRows as $row)
                                            ...
                                            <td>
                                                ...
                                                @elseif($row->type == 'image')
                                                    <img class="image-expandable" src="@if( !filter_var($data->{$row->field}, FILTER_VALIDATE_URL)){{ Voyager::image( $data->{$row->field} ) }}@else{{ $data->{$row->field} }}@endif" style="width: 60px; height: 60px; border-radius: 30px; margin-right: 10px; object-fit: cover;">
                                                ...
                                                @elseif($row->type == 'multiple_images')
                                                    ...
                                                    @if($images)
                                                        ...
                                                        @foreach($images as $image)
                                                            <img class="image-expandable" src="@if( !filter_var($image, FILTER_VALIDATE_URL)){{ Voyager::image( $image ) }}@else{{ $image }}@endif" style="width: 60px; height: 60px; border-radius: 30px; margin-right: 10px; object-fit: cover;">
                                                        @endforeach
                                                    @endif
                                                @elseif($row->type == 'media_picker')
                                                    ...
                                                    @if ($files)
                                                        @if (property_exists($row->details, 'show_as_images') && $row->details->show_as_images)
                                                            ...
                                                            <img class="image-expandable" src="@if( !filter_var($file, FILTER_VALIDATE_URL)){{ Voyager::image( $file ) }}@else{{ $file }}@endif" style="width: 60px; height: 60px; border-radius: 30px; margin-right: 10px; object-fit: cover;">
                                                            @endforeach
                                                        @else
                                                            <ul>
                                                            @foreach (array_slice($files, 0, 3) as $file)
