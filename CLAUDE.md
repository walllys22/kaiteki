...
- **Responsive por escala fluida, no por breakpoints.** Los tamanos salen de variables `clamp()` en `:root` (`--pad`, `--pad-bloque`, `--t-dojo`, `--t-nombre`, `--t-grado`, `--t-dato`, `--t-rotulo`): al agregar elementos, usar esas variables en vez de px fijos. Los `@media` solo cambian **disposicion**, no tamanos: 700px (bordes), 560px (ancho de la glosa), 520px (una columna + el veredicto se corre para no quedar bajo el sello), 380px (se oculta KARATE-DO), `orientation: landscape` con poca altura (recorta aire vertical) y 1100px (rollo mas ancho).
