...
### Dojo activo (selector del sidebar para el rol `administrador`)
El rol `administrador` elige en el sidebar que sucursal esta mirando. La eleccion se guarda en sesion y filtra **todo el sistema** sin tocar los controladores.

Comportamiento por rol:

| Rol | `users.dojo_id` | Selector | Alcance |
|-----|-----------------|----------|---------|
| `admin` | `null` | no | todas las sucursales, siempre |
| `administrador` | `null` | si | una sucursal a la vez (obligatorio) |
| `administrador_dojo` | seteado | no | su propia sucursal |

- `User::isGlobal()` — unica forma valida de preguntar "es usuario global" (`admin` o `administrador`). Lee `getRawOriginal('dojo_id')`. **No usar `$user->dojo_id === null` para esto**: ese accessor devuelve el dojo elegido.
- `User::isSuperAdmin()` — rol `admin`. Nunca se filtra por sucursal.
- `User::usaDojoActivo()` — `isGlobal() && ! isSuperAdmin()`, o sea solo `administrador`. Es el chequeo que gobierna el selector, el middleware y la vista.
- `User::getDojoIdAttribute()` — operador de sucursal: siempre su `dojo_id` real (la sesion se ignora, no puede cambiar de sucursal). `administrador`: el dojo elegido. `admin`: siempre `null`.
...
- Select en `resources/views/vendor/voyager/dashboard/sidebar.blade.php`, visible solo si `usaDojoActivo()`.

**No existe la opcion "Todos los dojos".** El rol `administrador` siempre esta parado en una sucursal concreta; nunca ve informacion mezclada de varias. La opcion global quedo comentada en el select del sidebar y en `ContextoDojoController@update` (que valida `dojo_id` como `required`). Para rehabilitarla hay que descomentar ambos lugares.

El middleware `EnsureDojoActivo` (alias `dojo.activo`, aplicado a todo el grupo `/admin`) garantiza la invariante: si un `administrador` no tiene dojo elegido — o el elegido fue dado de baja — le asigna el primer dojo activo por nombre. No toca ni al rol `admin` ni a los operadores de sucursal. Corre antes de los controladores, asi que la primera pantalla ya sale filtrada.
...
- `AlumnoGradoController::anularPagoRepaso()` / `anularPagoExamen()` — privilegio de usuario global, independiente del dojo elegido.

Cobertura de regresion: `tests/Feature/DojoActivoTest.php`.

El selector es una comodidad de UI, no un limite de seguridad: un `administrador` puede cambiar de sucursal cuando quiera, y el rol `admin` ve todo igual. El limite real sigue siendo `users.dojo_id` en base.
...
- **Cinta en SVG:** `resources/views/certificados/partials/cinta.blade.php`. Recibe `$cinta = ['base','franja','puntas','texto']`, que arma el closure `$resolverCinta()` de la vista a partir de `grados.nombre`. El color base se busca **antes** de la palabra `franja` y el de la franja **despues**: sin ese corte "Cinturon Blanco Franja Amarilla" resuelve a amarillo entero. Las "puntas" del grado marron (`Una/Dos/Tres Punta`) se dibujan como barras sobre el extremo colgante. Un nombre que no matchee cae a gris `#c9c2b4`; si se agregan colores nuevos a `grados`, sumarlos a `$paleta`.
- Estetica de dojo: papel washi, tinta sumi, sello bermellon redondo con texto en castellano. **Sin caracteres japoneses**: la pagina va integramente en alfabeto latino. El fallback del logo del dojo es su inicial.
- Transiciones de entrada encadenadas: el rollo se despliega (`desplegar`, con `clip-path`), las secciones suben escalonadas (`subir`), un destello barre el papel (`barrer`), el sello se estampa y suelta una onda (`estampar`, `onda`), la cinta se anuda y despues se mece en loop lento (`anudar`, `mecer`), y el retrato revela con un halo (`revelar`, `halo`).
- **Revelado por scroll** (pensado para celular, donde la pagina no entra en una pantalla): los bloques con clase `.revelable` se muestran al entrar en viewport via `IntersectionObserver`. El ocultamiento inicial esta bajo el selector `html.js .revelable`, y la clase `js` la agrega un script inline en el `<head>`: **sin JavaScript nada queda oculto**. Hay ademas un fallback en `load` que revela lo que quedo dentro de pantalla. No introducir reglas `.revelable` sin el prefijo `html.js` (hay un test que lo verifica).
- Todo el movimiento se anula en `prefers-reduced-motion: reduce` y en `@media print`, y el observer se saltea directamente en ese caso. Si se agregan animaciones nuevas hay que sumarlas a esos dos bloques.
- **Responsive por escala fluida, no por breakpoints.** Los tamanos salen de variables `clamp()` en `:root` (`--pad`, `--pad-bloque`, `--t-dojo`, `--t-nombre`, `--t-grado`, `--t-dato`, `--t-rotulo`, `--foto-an`, `--foto-al`): al agregar elementos, usar esas variables en vez de px fijos. El halo del retrato (`.alumno::before`) se dimensiona con `--foto-an` / `--foto-al`, asi que basta redefinirlas para cambiar el tamano de la foto y que el halo acompane.
- **En celular la hoja va a pantalla completa y toda la validacion entra sin scroll** (`max-width: 700px`): sin padding del body, sin bordes ni sombra, `min-height: 100svh` y el pie empujado con `margin-top: auto`. Lo mismo aplica a `invalido.blade.php`.
- Para que entre en una pantalla, en celular la escala se ata tambien a la **altura**: las variables usan `min(Nvw, Nsvh)`. Los coeficientes `svh` estan calibrados con mediciones reales (Chrome headless) y **no conviene subirlos**: si crecen, el contenido escala junto con la pantalla y vuelve a desbordar en los equipos bajos. Escalones de recorte: `max-height: 780px` oculta glosa, subtitulos de datos y KARATE-DO; `max-height: 640px` (iPhone SE) aprieta paddings, foto y cuerpos.
- En celular la foto va **a la izquierda** (no apilada) y los datos en **dos columnas**: apilar ambos sumaba ~200px de alto y sacaba el pie de la pantalla.
- Se usa `min-height`, nunca `height`: si el contenido no entra (equipo muy bajo, fuente del sistema agrandada), la pagina scrollea antes que recortar informacion del certificado.
- Medido sin desborde vertical ni horizontal en 375x667, 360x740, 390x844, 412x915 y 430x932, incluido el peor caso de nombre e instructor largos.
- Los demas `@media` solo cambian **disposicion**: 560px (ancho de la glosa), 520px (una columna, retrato mas grande y centrado, el veredicto se corre para no quedar bajo el sello), 380px (se oculta KARATE-DO), `orientation: landscape` con poca altura (recorta aire vertical) y 1100px (rollo mas ancho).
- Guardas de celular: `min-height: 100svh` con fallback a `100vh`, `overflow-x: hidden` en el body, `img, svg { max-width: 100% }`, y `-webkit-tap-highlight-color: transparent`.
- El numero de registro se muestra crudo (`{{ $regId }}`), sin ceros a la izquierda.
